...
export const SCENARIOS = [
  ...
  },
  {
    ...
    goals: ["Nói tên người đặt phòng", "Xác nhận ở ba đêm", "Hỏi giờ ăn sáng", "Hỏi mật khẩu wifi", "Hỏi giờ trả phòng"],
  },
  {
    id: "doctor-visit",
    level: "A2",
...
export function readSessions() {
  try {
    const raw = localStorage.getItem(scopedStorageKey(sessionsKey));
...
export function saveSession(session) {
  ...
  try {
    localStorage.setItem(scopedStorageKey(sessionsKey), JSON.stringify(next));
  } catch {

...
/** Giữ lại nội dung đủ để tạo bài luyện lại, thay vì chỉ lưu một nhãn thống kê. */
export function normaliseIssues(issues) {
  return (issues ?? []).slice(0, 3).map((issue) => ({
    kind: issue?.kind === "improvement" ? "improvement" : "error",
    type: normaliseErrorType(issue?.type),
    wrong: String(issue?.wrong ?? "").trim(),
    right: String(issue?.right ?? "").trim(),
    why: String(issue?.why ?? "").trim(),
    rule: String(issue?.rule ?? "").trim(),
    example: String(issue?.example ?? "").trim(),
    sourceSkill: String(issue?.sourceSkill ?? "writing").trim() || "writing",
  })).filter((issue) => issue.right || issue.why);
}

/**
 * Dựng một dòng nhật ký. Tách khỏi phần ghi đĩa để test được.
 *
 * Khai kiểu bằng JSDoc chứ không để TypeScript tự suy: file này là .mjs, mà mỗi
 * nơi gọi truyền một tập trường khác nhau — bài dịch không có practiceType, bài
 * nói không tự rút errorTypes. Không khai thì TS lấy chữ ký suy được ở một file
 * làm chuẩn cho mọi file, và nơi nào thiếu một trường là vỡ biên dịch.
 *
 * @param {{
 *   term?: string, vietnamese?: string, answer?: string, reference?: string,
 *   score?: number, correct?: boolean, gradedBy?: string,
 *   errorTypes?: string[] | null,
 *   issues?: Array<Record<string, unknown>> | null,
 *   assessedTypes?: string[] | null,
 *   practiceType?: string | null,
 * }} input
 * @param {Date} [now]
 */
export function makeAttempt({ term, vietnamese, answer, reference, score, correct, gradedBy,
  errorTypes = null, issues = null, assessedTypes = null, practiceType = null }, now = new Date()) {
  const cleanIssues = normaliseIssues(issues);
  const cleanErrorTypes = [...new Set((errorTypes?.length ? errorTypes : cleanIssues.map((issue) => issue.type)).map(normaliseErrorType))];
  const cleanPracticeType = practiceType ? normaliseErrorType(practiceType) : null;
  return {
    at: now.toISOString(),
    day: localDay(now),
    ...
    errorTypes: cleanErrorTypes,
    issues: cleanIssues,
    // assessedTypes cho biết loại lỗi nào thực sự được kiểm tra. Một bài luyện mạo
    // từ làm đúng không được tự động biến lỗi giới từ thành "đã sửa".
    assessedTypes: [...new Set((assessedTypes ?? (cleanPracticeType ? [cleanPracticeType] : cleanErrorTypes)).map(normaliseErrorType))],
    practiceType: cleanPracticeType,
  };
}

/** Lấy lần sửa gần nhất có đủ nội dung để mở bài Error Practice. */
export function practiceForError(entries, type) {
  const wanted = normaliseErrorType(type);
  for (const entry of [...(entries ?? [])].reverse()) {
    const issue = (entry.issues ?? []).find((item) => normaliseErrorType(item?.type) === wanted && item?.right);
    if (issue) return { ...issue, vietnamese: entry.vi ?? "", answer: entry.answer ?? "", reference: entry.reference ?? "" };
  }
  return null;
}
...
export function readAttempts() {
  try {
    const raw = localStorage.getItem(scopedStorageKey(attemptsKey));
...
export function logAttempt(attempt) {
  ...
  try {
    localStorage.setItem(scopedStorageKey(attemptsKey), JSON.stringify(entries));
  } catch {

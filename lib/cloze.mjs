...
export function clozeFor(term, example) {
  ...
  for (const candidate of candidates) {
    ...
    // Thử ranh giới từ trước; không được mới chấp nhận khớp biến thể, vì có những
    // từ khoá chỉ xuất hiện dưới dạng biến thể ("fly" trong "flying"). Khi khớp
    // biến thể thì phải nuốt cả phần đuôi: để lại "_____d" trong câu vừa lộ đáp án
    // vừa khiến người học không biết phải điền dạng nào.
    for (const pattern of [`\\b${escaped}\\b`, `\\b${escaped}[a-zA-Z]*`, escaped]) {

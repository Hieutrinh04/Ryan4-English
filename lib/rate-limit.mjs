...
export function createLimiter({ windowMs = 60_000, burst = 6, dailyQuota = 60 } = {}) {
  ...
  return {
    ...
    /** Xem còn bao nhiêu lượt hôm nay mà KHÔNG trừ. */
    peek(key, now = Date.now()) {
      const bucket = buckets.get(key);
      if (!bucket || bucket.day !== dayOf(now)) return { used: 0, remainingToday: dailyQuota, dailyQuota };
      return { used: bucket.used, remainingToday: Math.max(0, dailyQuota - bucket.used), dailyQuota };
    },

...
/**
 * Dấu đứng đầu phiên âm do mô hình đoán, không phải lấy từ từ điển.
 *
 * Nhớ chung một chỗ với phiên âm thật cho gọn, nhưng phải phân biệt được: người
 * học cần biết chỗ nào là tra ra và chỗ nào là phỏng đoán, nhất là với tên riêng.
 */
...
export function withIpa(text, cache) {
  ...
  return String(text ?? "")
    ...
    .map((token) => {
      ...
      const guessed = stored.startsWith(GUESS_MARK);
      return {
        word: token,
        ipa: stored === NO_IPA ? "" : guessed ? stored.slice(GUESS_MARK.length) : stored,
        isWord: Boolean(key),
        checked: Boolean(stored),
        // Giao diện phải nói rõ đây là phỏng đoán chứ không phải tra từ từ điển.
        estimated: guessed,
      };
    });
}
...
function numberRow(token, store) {
  ...
  const found = parts.map((part) => {
    const value = String(store[wordKey(part)] ?? "");
    return value.startsWith(GUESS_MARK) ? value.slice(GUESS_MARK.length) : value;
  });
 ...
 */
export function saveIpa(found, missing = [], estimated = {}) {
  ...
  // Phiên âm đoán chỉ dùng khi không có bản tra được: tra ra bao giờ cũng hơn đoán.
  for (const [word, ipa] of Object.entries(estimated ?? {})) {
    const key = wordKey(word);
    const value = String(ipa ?? "").trim();
    const cu = String(next[key] ?? "");
    if (key && value && (!cu || cu === NO_IPA)) next[key] = GUESS_MARK + value;
  }

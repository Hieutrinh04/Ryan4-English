...
export function scoreShadowing(target, heard) {
  ...
  if (!reference.length) return { clarity: 0, words: [], marks: [], missed: [], swallowed: [], extra: [], spokenCount: spoken.length };

  const operations = alignTokens(reference, spoken);
  // Trong một cụm sai liên tiếp, ghép từ câu mẫu bị thiếu với từ máy nghe thừa
  // theo đúng thứ tự. Nhờ vậy UI có thể nói rõ "skateboarding" đã bị nghe thành
  // "spike", thay vì chỉ hiện hai chip rời rạc không giải thích quan hệ.
  for (let start = 0; start < operations.length;) {
    if (operations[start].type === "same" || operations[start].type === "form") {
      start++;
      continue;
    }
    let end = start;
    while (end < operations.length && operations[end].type !== "same" && operations[end].type !== "form") end++;
    const missing = operations.slice(start, end).filter((item) => item.type === "missing");
    const extras = operations.slice(start, end).filter((item) => item.type === "extra");
    for (let index = 0; index < Math.min(missing.length, extras.length); index++) missing[index].heard = extras[index].answer;
    start = end;
  }
  const words = [];
  const extra = [];
  // marks giữ NGUYÊN thứ tự nói, kể cả từ thừa. words thì chỉ có từ của câu mẫu.
  // Cần cả hai: chấm điểm dựa trên câu mẫu, còn hiện lên màn hình thì phải thấy
  // từ thừa nằm đúng chỗ nó chen vào, nếu không người học không hiểu vì sao sai.
  const marks = [];
  for (const operation of operations) {
    if (operation.type === "same") {
      words.push({ word: operation.reference, status: "ok" });
      marks.push({ word: operation.reference, status: "ok" });
    } else if (operation.type === "form") {
      words.push({ word: operation.reference, heard: operation.answer, status: "swallow" });
      marks.push({ word: operation.reference, heard: operation.answer, status: "swallow" });
    } else if (operation.type === "missing") {
      words.push({ word: operation.reference, heard: operation.heard, status: "missed" });
      marks.push({ word: operation.reference, heard: operation.heard, status: "missed" });
    } else {
      extra.push(operation.answer);
      marks.push({ word: operation.answer, status: "extra" });
    }
  }

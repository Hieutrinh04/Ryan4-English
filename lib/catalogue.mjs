import { catalogueEntry, groupByChannel, usableForPractice } from "./youtube-list.mjs";
...
export function readCatalogue() {
  try {
    const raw = JSON.parse(localStorage.getItem(scopedStorageKey(catalogueKey)) ?? "[]");
    if (!Array.isArray(raw)) return [];
    // Lọc ngay lúc đọc, không chỉ lúc thêm: danh mục của người dùng đã có sẵn
    // hàng trăm video thêm từ trước khi có các bộ lọc này. Không lọc ở đây thì
    // họ phải tự xoá từng cái. Dùng chung usableForPractice để một luật duy nhất
    // quyết định video nào dùng được — Shorts, quá ngắn và quá dài cùng một chỗ.
    return raw.filter((item) => item?.videoId && item?.title && usableForPractice(item));
  } catch {
    return [];
  }
}
...
function write(list) {
  try {
    localStorage.setItem(scopedStorageKey(catalogueKey), JSON.stringify(list));
  } catch {
...
/**
 * Đưa bài đã có phụ đề vào danh mục để thư viện thật sự dựng thẻ cho nó.
 *
 * Kho bài (`lessons`) và danh mục video (`catalogue`) được lưu riêng. Trước đây
 * tiện ích chỉ ghi kho bài nên hiện thông báo thành công, nhưng thư viện không
 * có mục tương ứng để hiển thị. Tên kênh trong bài là `author`, còn danh mục gọi
 * trường đó là `channel`, nên chuẩn hoá tại một chỗ để mọi đường nhập dùng chung.
 */
export function addLessonsToCatalogue(lessons) {
  const videos = (Array.isArray(lessons) ? lessons : []).map((lesson) => ({
    videoId: lesson?.videoId,
    title: lesson?.title,
    channel: lesson?.author,
    seconds: lesson?.seconds,
  }));
  return addToCatalogue(videos);
}
...
/**
 * Ghép danh mục với bài thật và tiến độ của nó.
 *
 * Chỉ trả cờ `ready` là chưa đủ: giao diện cần chính `lesson` để mở màn học.
 * Thiếu trường này khiến thẻ vừa nhập vẫn ghi “Chưa có phụ đề” và bấm vào lại
 * mở YouTube ngoài dù kho bài đã lưu đủ câu.
 */
export function withLessonState(entries, lessons, progress, mode = "shadowing") {
  const byId = new Map((Array.isArray(lessons) ? lessons : []).map((item) => [item.videoId, item]));
  return (Array.isArray(entries) ? entries : []).map((entry) => {
    const lesson = byId.get(entry.videoId) ?? null;
    return { ...entry, ready: !!lesson, lesson, ...videoProgress(lesson, progress, mode) };
  });
}

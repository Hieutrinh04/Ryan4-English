...
// Đề bài là tiếng Việt nên tuyệt đối không được chép nguyên từ tiếng Anh cần dịch
// vào câu: vừa thành câu lai ("để run sức khỏe"), vừa làm lộ đáp án. Chuẩn hoá về
// các cụm chữ/số có khoảng trắng để so đúng cả từ đơn lẫn cụm từ, kể cả tiếng Việt
// có dấu ở hai bên.
const searchablePhrase = (value) => String(value ?? "").normalize("NFC").toLocaleLowerCase().replace(/[^\p{L}\p{N}]+/gu, " ").trim();

export function containsTargetTerm(vietnamese, terms) {
  const haystack = ` ${searchablePhrase(vietnamese)} `;
  return (Array.isArray(terms) ? terms : [terms]).some((term) => {
    const variants = String(term ?? "").split("/").map(searchablePhrase).filter(Boolean);
 ...
 * @param {{ areas?: [string, string[]][]; size?: number; minSize?: number }} [options]
 *
 * minSize > 1: gộp các đoạn quá ngắn lại. Khi người học chọn "một đoạn liền mạch"
 * mà 13 từ rải ra 10 chủ đề, tách theo chủ đề sẽ ra 10 đoạn một câu — nhìn y hệt
 * "từng câu riêng". Đặt minSize để dồn thành vài đoạn thật sự dài.
 */
export function buildPassages(tasks, options = {}) {
  const { areas = [], size = PASSAGE_SIZE, minSize = 0 } = options;
  ...
  if (minSize > 1) {
    // Giữ thứ tự đã gom theo chủ đề nhưng cắt thành đoạn đúng `size` câu, không
    // để chủ đề nhỏ tự thành một đoạn. Đoạn cuối ngắn quá thì dồn lên đoạn trước.
    const flat = groups.flatMap(([topic, group]) => group.map((task) => ({ task, topic })));
    const merged = [];
    for (const { task, topic } of flat) {
      let current = merged[merged.length - 1];
      if (!current || current.tasks.length >= size) {
        current = { topic, tasks: [] };
        merged.push(current);
      }
      current.tasks.push(task);
    }
    if (merged.length > 1 && merged[merged.length - 1].tasks.length < minSize) {
      merged[merged.length - 2].tasks.push(...merged.pop().tasks);
    }
    return merged;
  }

...
export const DRILL_MODES = [
  { value: "card", label: "Thẻ flashcard", icon: "cards", hint: "Lật thẻ rồi tự đánh giá đã thuộc hay chưa" },
  { value: "type", label: "Gõ từ", icon: "keyboard", hint: "Gõ lại từ tiếng Anh từ trí nhớ" },
  { value: "listen", label: "Nghe", icon: "volume", hint: "Nghe phát âm rồi gõ lại từ" },
  { value: "reverse", label: "Đảo ngược", icon: "swap", hint: "Xem từ tiếng Anh rồi nhớ lại nghĩa tiếng Việt" },
  { value: "quiz", label: "Trắc nghiệm", icon: "target", hint: "Chọn đúng nghĩa tiếng Việt của từ tiếng Anh" },
  { value: "mixed", label: "Hỗn hợp", icon: "shuffle", hint: "Trộn ngẫu nhiên các cách trên cho khỏi đoán trước" },
];

/** Các chế độ mà "Hỗn hợp" bốc ngẫu nhiên. Không bốc "card" vì nó không chấm được. */
export const MIXED_POOL = ["type", "listen", "reverse", "quiz"];
...
export function supportsMode(card, mode) {
  ...
  if (mode === "reverse" || mode === "quiz") return Boolean(card.meaning);

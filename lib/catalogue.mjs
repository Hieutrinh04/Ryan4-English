import { catalogueEntry, groupByChannel, looksLikeShort } from "./youtube-list.mjs";
...
export function readCatalogue() {
  try {
    ...
    // Lọc Shorts ngay lúc đọc, không chỉ lúc thêm: danh mục của người dùng đã có
    // sẵn hàng trăm video thêm từ trước khi có bộ lọc này. Không lọc ở đây thì
    // họ phải tự xoá từng cái.
    return raw.filter((item) => item?.videoId && item?.title && !looksLikeShort(item));
  } catch {
    return [];
  }
}

...
function migrateSpeaking(store) {
  try {
    if (localStorage.getItem(scopedStorageKey(practiceMigratedKey))) return store;
    const raw = localStorage.getItem(scopedStorageKey(speakingKey));
    const old = raw ? JSON.parse(raw) : null;
    localStorage.setItem(scopedStorageKey(practiceMigratedKey), new Date().toISOString());
    ...
    localStorage.setItem(scopedStorageKey(practiceKey), JSON.stringify(merged));
    return merged;
  } catch {
    return store;
  }
...
export function readPractice() {
  try {
    const raw = localStorage.getItem(scopedStorageKey(practiceKey));
...
export function logPractice(skill, seconds) {
  ...
  try {
    localStorage.setItem(scopedStorageKey(practiceKey), JSON.stringify(trimmed));
  } catch {
